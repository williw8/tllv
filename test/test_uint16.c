...
int test_uint16(test_node_t *rec)
{
    ...
    uint16_t chk;
    uint16_t test_values[] = {0,1,128,255,256,65534,65535};
    tllv_t *tllv;
    ...
    tllv = create_tllv();
    if (tllv) {
        for (test_value_idx=0;test_value_idx<(sizeof(test_values)/sizeof(uint16_t));test_value_idx++) {
            tllv_insert_uint16(tllv,test_values[test_value_idx],&err); 
            if (err) {
                rv = err;
                goto out;
	    }
            chk = tllv_extract_uint16(tllv,&err);
	    line = __LINE__ - 1;
            if (err) {
                rv = err;
                goto out;
